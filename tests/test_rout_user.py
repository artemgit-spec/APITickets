...
def test_create_user(client):
    resp = client.post('/users/reg-user',
                       json={"name":"user2","password":"user2","mail_user":"user2@example.com","is_admin": "user"})
    assert resp.status_code == 200
    assert resp.json() == {"status_code": 201,"detail": "Пользователь создан"}
    

@pytest.mark.parametrize("auth_token, id, status, answer",[
    ("admin", 1, 200, {"id": 1,"name": "admin","is_admin": "admin","mail_users": "user@example.com"}),
    ("admin", 100, 404, {"detail":"Нет такого пользователя"}),
    ("user", 2, 200, {"id": 2,"name": "user","is_admin": "user","mail_users": "user@example.com"}),
...
@pytest.mark.parametrize("auth_token, id, new_status, status, answer", [
...
def test_update_status(auth_token, id, new_status, status, answer, client):
    resp = client.patch(f'/users/update-status-user/{id}',
