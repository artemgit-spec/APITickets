from fastapi import APIRouter, Depends, status, HTTPException, Body, Query, Form
from fastapi.responses import JSONResponse
from typing import Annotated, List
from enum import Enum
from datetime import datetime

from db_management.db import Ticket, User, session_db
from sqlalchemy import select, insert, update, delete
from sqlalchemy.orm import Session

from schemas.schemas_tickets import CreateTickets, TicketModel, UpdateTicket, UpdateStatusTicket, InfoTicket
...
async def create_tickets(
    ...
    db.execute(insert(Ticket).values(
...
async def update_tickets(
    ...
    if payload.get('is_admin') == 'admin':
        ...
        if up_tic.title == "string" and up_tic.text_ticket == "string":
            ...
            db.execute(update(Ticket).where(Ticket.id==id).values(
                title = up_tic.title,
...
async def status_update(
    ...
    if payload.get("is_admin") == "admin":
        db.execute(update(Ticket).where(Ticket.id == id).values(
            ...
            closing_ticket=datetime.now()
    ))
        db.commit()
        return {'status_code':status.HTTP_200_OK,
                'detail':'Тикет обновлен'}
